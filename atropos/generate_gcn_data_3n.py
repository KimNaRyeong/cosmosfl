...
def main(model):
    ...
    for k in ks:
        ...
        if not os.path.exists(f"./data/{model}/R10_N3/{k}"):
            os.makedirs(f"./data/{model}/R10_N3/{k}")
    ...
    for i in range(1, 4):
        ...
        combined_result_file = f"../autofl/weighted_fl_results/R10_N3/accat1_de_equal{i}.json"
    ...
    with open(f'./data/R10_N3/arg_vector_size.txt', 'w') as f:
        for k in ks:
    ...
    for k in ks:
        if not os.path.exists(f"./data/R10_N3/{k}"):
            os.makedirs(f"./data/R10_N3/{k}")

        torch.save({
            "dataset_S": all_gcn_S[k],
            "dataset_F": all_gcn_F[k],
            "dataset_FA": all_gcn_FA[k],
        }, f"data/R10_N3/{k}/gcn_dataset.pth")
