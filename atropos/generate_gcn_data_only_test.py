...
def generate_LIG(reasoning_paths_dict, labels_dict, args_dict, k, arg_vector_size, model, r, n, file_idx):
...
def main(model, repetition, num_files):
    ...
    with open(f'./data/{model}/R{repetition}_{num_files}files/test_bug_list', 'r') as f:
        test_bug_list = f.read().splitlines()

    max_arg_vec_size_dict = dict()
    
    with open(f'./data/{model}/R{repetition}_{num_files}files/arg_vector_size.txt', 'r') as f:
        for l in f.read().splitlines():
            k, length = l.split()
            max_arg_vec_size_dict[int(k)] = int(length)
    ...
    for i in range(1, num_files+1):
        ...
        for bug_name in tqdm(test_bug_list):
    ...
    for k in ks:
        for i in range(1, num_files+1):
            test_gcn_S, test_gcn_F, test_gcn_FA = generate_LIG(test_reasoning_paths_dict[i][k], labels_dict[i], args_dict[i][k], k, max_arg_vec_size_dict[k], model = model, r= repetition, n = num_files, file_idx = i)
